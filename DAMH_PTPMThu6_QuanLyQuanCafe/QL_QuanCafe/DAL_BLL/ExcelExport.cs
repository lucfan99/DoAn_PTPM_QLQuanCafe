...
namespace DAL_BLL
{
    public class ExcelExport
    {
        ...
        public const string T_DanhMucKhoa = "HienThiKhoa";
        public const string T_HoaDon = "HienThi";
        #endregion
        ...
        private void BuildReplacerCurrentDate(ref Dictionary<string, string> pReplacer, string mahd, string ngaylap, string ban, string nv, double tongtien)
        {
            if (pReplacer != null)
            {
                pReplacer.Add("%MaHD", mahd);
                pReplacer.Add("%NgayThangNam", ngaylap);
                pReplacer.Add("%NhanVien", nv);
                pReplacer.Add("%Ban", ban);
                //pReplacer.Add("%ThanhTien", tongtien.ToString());
            }
        }
        //private void BuildReplacerCurrentDate(ref Dictionary<string, string> pReplacer, int pValue)
        //{
        //    if (pReplacer != null)
        //    {
        ...
        //        pReplacer.Add("%TongSo", pValue.ToString());
        //    }
        //}
        ...
        private MemoryStream GetTemplateStream(string viewName)
        {
            ...
            switch (viewName)
            {
                ...
                case T_DanhMucKhoa:
                    // path += Constants.FILE_KEHOACHTD;
                    arrByte = File.ReadAllBytes("HienThiKhoa.xls").ToArray();
                    break;
                case T_HoaDon:
                    // path += Constants.FILE_KEHOACHTD;
        ...
        //public bool ExportHoaDon(List<HOADON> dataSource, ref string fileName, bool isPrintPreview,string mahd,string ngaylap,string ban,string nv,string tongtien)
        //{
        //    // Check if data is null
        ...
        //    BuildReplacerCurrentDate(ref replacer, mahd,ngaylap,ban,nv,tongtien);

        //    return OutSimpleReport(dataSource, replacer, "HienThi", isPrintPreview, ref fileName);
        //}
        public bool ExportKhoa(List<INHOADON> dataSource, ref string fileName, bool isPrintPreview, string mahd, string ngaylap, string nv, string ban, double tongtien)
        {
            // Check if data is null
            if (dataSource == null || (dataSource != null && dataSource.Count == 0))
            {
